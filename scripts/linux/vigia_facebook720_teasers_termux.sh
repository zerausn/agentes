...
# ================================================================
# 4_VIGIA_FACEBOOK720_TEASERS — Evacuador Facebook TEASERS (Note9 Termux)
...
SESSION_LOG="$LOG_DIR/4_VIGIA_FACEBOOK720_TEASERS.log"
...
wait_until() {
    ...
    objetivo=$(date -d "@${target_epoch}" '+%H:%M:%S' 2>/dev/null \
               ...
               || echo "??:??:??")
    echo "[TEASERS ESPERA] Proxima subida a las: ${objetivo}"
    ...
    while true; do
        ...
        if [ "$diff" -le 0 ]; then
            echo "[TEASERS RELOJ] Hora alcanzada: $(date '+%H:%M:%S') — arrancando ciclo."
            return 0
        fi

        printf "\r[TEASERS RELOJ] %3ds restantes (objetivo %s)..." "$diff" "$objetivo"
...
echo ""
echo "=============================================="
echo "  4_VIGIA_FACEBOOK720_TEASERS — reloj sistema"
...
trap 'printf "\n"; echo "[SALIDA TEASERS] $(date '+%H:%M:%S') — liberando wake-lock"; termux-wake-unlock 2>/dev/null || true; exit' INT TERM EXIT
...
while true; do
    ...
    printf "\n"
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo "  CICLO TEASERS #${CICLO} — $(date '+%Y-%m-%d %H:%M:%S')"
    echo "  Destino: Shirabyoshi Writings (1347014641828725)"
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
...
export META_FB_PAGE_TOKEN
    ...
    case "$EXIT_CODE" in
        0)  echo "[TEASERS CICLO #${CICLO}] OK — subida exitosa en ${DURACION}s. | Pendientes TEASERS: ${PENDIENTES}"
            ;;
        2)  echo "[TEASERS CICLO #${CICLO}] Sin TEASERS pendientes (${DURACION}s). | Pendientes: 0"
            ;;
        *)  echo "[TEASERS CICLO #${CICLO}] Error exit=$EXIT_CODE (${DURACION}s). | Pendientes TEASERS: ${PENDIENTES}"
            ;;
    esac

    # Calcular próxima ejecución basada en reloj real
    NEXT_EPOCH=$(( T_FIN + INTERVALO ))
    echo "[TEASERS RELOJ] Ciclo terminó: $(date '+%H:%M:%S') | Siguiente en ${INTERVALO}s"
