...
# ================================================================
# 5_VIGIA_FACEBOOK720_BIFURCADO — Evacuador Facebook BIFURCADO (Note9 Termux)
# Sube 1 video cada 720 segundos.
#
# BIFURCACIÓN:
#   - Videos CRUDOS  → Performatic Writings Cali (ID: 803559979506784)
...
EVACUADOR_PROOT="$PR_ROOT/root/agentes/meta_uploader/subir_fb_evacuador_bifurcado.py"
LOG_FILE="$PR_ROOT/root/agentes/meta_uploader/fb_evacuador_bifurcado.log"
LOG_DIR="/sdcard/Antigravity/widget_logs"
SESSION_LOG="$LOG_DIR/5_VIGIA_FACEBOOK720_BIFURCADO.log"
...
echo ""
echo "=============================================="
echo "  5_VIGIA_FACEBOOK720_BIFURCADO — reloj sistema"
...
if [ ! -f "$EVACUADOR_PROOT" ]; then
    echo "[ERROR] No existe subir_fb_evacuador_bifurcado.py"
...
while true; do
    ...
    echo "  Crudos → Performatic Writings Cali (803559979506784)"
    echo "  Teasers → Shirabyoshi Writings (1347014641828725)"
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
...
source "$(dirname "$0")/_proot_bind.sh"
    "$PROOT" login debian "${PROOT_BIND_ARGS[@]}" -- /bin/bash -lc \
        "set -o pipefail; cd /root/agentes/meta_uploader && \
         ...
         python3 subir_fb_evacuador_bifurcado.py 2>&1 | tee -a '${LOG_FILE}'"
    EXIT_CODE=$?
