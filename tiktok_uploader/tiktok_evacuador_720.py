...
# BUG FIX: habia dos definiciones de NEXT_RE; se elimino la primera (mas restrictiva).
...
_LOCK_STALE_MINUTES = 30  # Lock mas antiguo que esto con PID muerto => zombie


def _lock_is_stale() -> bool:
    """Retorna True si el lock pertenece a un proceso muerto o tiene mas de
    _LOCK_STALE_MINUTES. Permite autorecuperacion sin intervencion manual."""
    try:
        content = LOCK_FILE.read_text(encoding="utf-8").strip()
        pid = int(content.split()[0])
        try:
            os.kill(pid, 0)  # signal 0: solo verifica existencia, no mata
            # Proceso vivo — verificar edad del lock
            age_secs = time.time() - LOCK_FILE.stat().st_mtime
            if age_secs > _LOCK_STALE_MINUTES * 60:
                logging.warning(
                    "Lock de PID %d lleva %.0f min — considerado zombie.",
                    pid, age_secs / 60,
                )
                return True
            return False  # instancia real corriendo
        except ProcessLookupError:
            logging.warning("Lock de PID %d (proceso muerto) — stale lock.", pid)
            return True
        except PermissionError:
            # Proceso de otro usuario: usar edad como criterio
            age_secs = time.time() - LOCK_FILE.stat().st_mtime
            return age_secs > _LOCK_STALE_MINUTES * 60
    except Exception as exc:
        logging.warning("No se pudo leer lock file: %s — asumiendo stale.", exc)
        return True
...
def process_lock():
    ...
    # Intentar crear el lock; si ya existe, verificar si es zombie
    for attempt in range(2):
        try:
            ...
            if attempt == 0 and _lock_is_stale():
                logging.warning("Lock stale detectado — eliminando y reintentando.")
                try:
                    os.unlink(lock_path)
                except OSError:
                    pass
...
def automate_tiktok_publish_coords(video: Path, caption: str, folder_name: str) -> bool:
    ...
    if PUBLISH_MODE == "draft":
        ...
        # BUG FIX: el loop anterior llamaba publish_confirmed dentro Y fuera del
        # while, triplicando el tiempo de espera. Se simplifica a una sola llamada.
        if publish_ok:
