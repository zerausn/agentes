"""
subir_fb_evacuador_bifurcado.py  (VERSION BASH-LOOP — sin time.sleep interno)
...
BIFURCACIÓN:
- Videos CRUDOS  → Página: Performatic Writings Cali (ID: 803559979506784)
- TEASERS        → Página: Shirabyoshi Writings (ID: 1347014641828725)
...
LOG_FILE   = BASE_DIR / "fb_evacuador_bifurcado.log"
...
# Tokens por página (fallback al token genérico si no están definidos)
...
def upload_video(video_path: Path) -> bool:
    ...
    # Seleccionar página y token según tipo de video
    if is_teaser:
    ...
    # TEASERS siempre como VIDEO ESTÁNDAR (Shirabyoshi no permite Reels con este token)
    if is_teaser:
...
def main():
    logging.info("=" * 60)
    logging.info("  FB EVACUADOR BIFURCADO (1 video/ciclo) — carpeta: %s", SOURCE_DIR)
