...
EXTERNAL_SUCCESS_DIR = Path("/media/zerausn/D69493CF9493B08B/Users/ZN-/Documents/ADM/Carpeta 1/ya_subidos_fb_ig")
EXTERNAL_TEMP_SUCCESS_DIR = Path("/media/zerausn/D69493CF9493B08B/Users/ZN-/Documents/ADM/Carpeta 1/ya_subidos_ig_temp")
...
def sync_local_queues_with_remote(posts, reels, *, max_pages=150):
    logging.info("Sincronizando colas locales contra Facebook...")
    fb_cache = get_facebook_library_batch(max_pages=max_pages)
    
    if fb_cache is None:
        logging.error("CRITICO: Fallo la descarga del catalogo remoto de Facebook. Abortando por seguridad.")
        raise SystemExit("Error de sincronizacion preventivo.")

    remote_library = tuple(text for text in fb_cache if text)
...
def extract_asset_marker(video_path):
    ...
    if match:
        base = f"{match.group('date')}_{match.group('time')}"
        # Buscar especificamente si tiene sufijo de teaser (ej: _teaser_001) para evitar colisiones
        teaser_match = re.search(r"(?i)[_#](teaser[_#\s-]*\d+)", stem)
        if teaser_match:
            # Normalizamos a 'teaser_N' para el marcador
            clean_teaser = re.sub(r"[#\s-]", "_", teaser_match.group(1)).lower()
            return f"{base}_{clean_teaser}"
        return base
    return stem
...
def _invoke_with_status(label, func, *args):
    try:
        ...
        result = None
    # Salvaguarda: Asegurar que el resultado sea serializable en JSON
    if result is not None:
        try:
            json.dumps(result)
        except TypeError:
            logging.warning("Resultado de %s no serializable: %s. Convirtiendo a string.", label, type(result))
            result = str(result)
...
def run_platform_pair(lane_name, video_info, publish_date, target_time_str=None):
    # --- DETECCION DE TEASER ---
    import re
    teaser_match = re.search(r"(?i)[_#]teaser[_#\s-]*(\d+)", video_info["filename"])
    is_teaser = bool(teaser_match)
    teaser_num = teaser_match.group(1) if is_teaser else None
    
    if is_teaser:
        logging.info("  [MODO TEASER #%s] %s detectado. Publicacion inmediata.", teaser_num, video_info["filename"])
        scheduled_timestamp = None
        type_tag = f"teaser #{teaser_num}"
    else:
        type_tag = "full"
        if target_time_str:
    ...
    # (Ya definido arriba)
    caption = build_caption(video_info, lane_name, publish_date, type_tag)
    ...
    # --- LOGICA FACEBOOK UNIFICADA (Teaser Inmediato o Full Programado) ---
    search_tag = type_tag
    existing_fb, _ = _check_remote(caption_marker, search_tag)

    if existing_fb:
        logging.warning("[%s] Ya existe version '%s' en Facebook para %s. Saltando.", fb_label.upper(), search_tag, caption_marker)
    else:
        if is_teaser:
            logging.info("[%s] Publicando Teaser inmediato para Facebook...", fb_label.upper())
            work[f"{fb_label}_teaser_now"] = (upload_fb_reel, video_info["path"], caption, None, True, False, True) # background=True
        else:
            logging.info("[%s] Programando Video completo para %s @ %sh...", fb_label.upper(), publish_date, hour)
            work[f"{fb_label}_full_scheduled"] = (upload_fb_video_standard, video_info["path"], caption, scheduled_timestamp, True, False, True) # background=True
...
def execute_plan(plan, pause_between_assets=10, max_live_days=1):
    ...
    def _burst_worker(day_entry):
        ...
        for lane_name, video_info in tasks:
            ...
            # Extraer hilos de verificacion en segundo plano (Meta Asincrono)
            for res in summary.values():
                val = res.get("result")
                if isinstance(val, tuple) and len(val) == 2:
                    vid_id, vthread = val
                    active_verifiers.append(vthread)
                    res["result"] = str(vid_id) # Normalizar para JSON
    ...
    # --- ESPERA FINAL DE VERIFICADORES ASINCRONOS ---
    if active_verifiers:
        logging.info("Esperando a que Meta procese %s videos en segundo plano...", len(active_verifiers))
        for vt in active_verifiers:
            if vt.is_alive():
                vt.join()
        logging.info("Todos los procesos de segundo plano de Meta han finalizado.")
