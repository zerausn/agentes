...
def schedule_platform_pair(day_entry, lane_name, video_info):
    ...
    # Lógica de detección de Teaser para publicación inmediata
    import re
    teaser_match = re.search(r"(?i)[_#]teaser[_#\s-]*(\d+)", caption_marker)
    ...
    if is_teaser:
        logging.info("  [MODO TEASER #%s] %s detectado. Se forzara publicacion INMEDIATA.", teaser_num, video_info["filename"])
        fb_slot = build_slot_payload(fb_time, fb_label) # Mantenemos el slot para el registro local
        fb_scheduled_unix = None # Pero enviamos None a la API para que sea inmediato
    else:
        fb_slot = build_slot_payload(fb_time, fb_label)
        fb_scheduled_unix = fb_slot["scheduled_unix"]
    ...
    if existing_facebook:
        ...
        if lane_name == "reel":
            fb_result = upload_fb_reel(
                video_info["path"],
                caption,
                scheduled_publish_time=fb_scheduled_unix,
                is_draft=False
            )
        else:
