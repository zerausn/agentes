...
FB_UPLOAD_CHUNK_BYTES = int(os.environ.get("META_FB_UPLOAD_CHUNK_BYTES", str(16 * 1024 * 1024)))
        ...
        if error_payload:
            ...
            if "reduce the amount of data" in message or code == 1:
                if current_limit_val > 1:
                    ...
                    logging.warning("Paginacion profunda agotada o límite mínimo rechazado por Meta (página %s). Deteniendo iteración de forma segura para salvar los datos ya obtenidos.", pages)
                    break
            ...
            if _should_retry_http_status(response.status_code) and attempt < HTTP_RETRY_ATTEMPTS:
                ...
                if "reduce the amount of data" in json.dumps(payload).lower():
                    ...
                    _set_operation_status(
                        f"http_{response.status_code}_heavy",
                        "request_json",
                        json.dumps(payload, ensure_ascii=False),
                        transient=False,  # Marcamos como NO transitorio para forzar fresh retry
                    )
                    return payload
...
def ensure_ig_compatibility(file_path, max_duration=None, force_recode=False):
    ...
    # Usamos -map 0:v:0 -map 0:a:0? para asegurar que el video sea el primer stream
    # y evitar que ffmpeg a veces solo copie audio en ciertos contenedores/escenarios.
    cmd = ["ffmpeg", "-y", "-i", file_path, "-map", "0:v:0", "-map", "0:a:0?", "-movflags", "+faststart"]
    
    # Si pedimos max_duration o force_recode, aplicamos el perfil de compatibilidad maxima (Deep Clean)
    # Por experiencia, usar -c copy con -t (slicing) a veces pierde la pista de video en MP4.
    if force_recode or max_duration:
        cmd += [
            "-c:v", "libx264",
            "-preset", "veryfast",
            "-crf", "23",
        ...
        # Copia directa (Quick Clean) solo si no hay recortes ni recodificacion forzada
        cmd += ["-c", "copy"]
