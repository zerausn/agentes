    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            try:
                ...
                logging.warning(f"Token {key_index} expirado, solicitando nueva autenticacion...")
...
def main():
    parser = argparse.ArgumentParser(description="Nudge de videos atascados en YouTube")
    ...
    while current_idx < len(client_files) and not is_client_available(client_files[current_idx]):
        logging.info(f"Saltando llave {current_idx} por cuota agotada...")
        current_idx += 1
