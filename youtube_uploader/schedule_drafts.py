...
class YouTubeServicePool:
    ...
    def is_client_available(self, client_name):
        quota_file = BASE_DIR / "quota_status.json"
        if not quota_file.exists(): return True
        try:
            status = json.loads(quota_file.read_text(encoding="utf-8"))
    ...
    def update_quota_status(self, client_name):
        quota_file = BASE_DIR / "quota_status.json"
        status = {}
        if quota_file.exists():
            try: status = json.loads(quota_file.read_text(encoding="utf-8"))
            except: pass
        status[client_name] = {
            "last_quota_exceeded": datetime.now().isoformat(),
            "date": datetime.now().strftime("%Y-%m-%d"),
        }
        quota_file.write_text(json.dumps(status, indent=4), encoding="utf-8")

    def get_service(self):
        while self.current_idx < len(self.services):
            name, youtube = self.services[self.current_idx]
            if self.is_client_available(name):
                return youtube, name
            logging.info(f"Saltando {name} por quota agotada previa.")
            self.current_idx += 1
        return None, None

    def rotate(self, name):
        self.update_quota_status(name)
        self.current_idx += 1
