...
TEASER_SUFFIX_RE = re.compile(r"(?i)_teaser_(\d+)$")
...
def normalize_video_stem(raw_value) -> str:
...
def extract_teaser_sequence(raw_value):
    stem = Path(str(raw_value or "")).stem.strip()
    if not stem:
        return "", 1

    match = TEASER_SUFFIX_RE.search(stem)
    if not match:
...
def build_teaser_sort_key(raw_value):
    source_stem, teaser_number = extract_teaser_sequence(raw_value)
    normalized_name = Path(str(raw_value or "")).name.lower()
    return source_stem.lower(), teaser_number, normalized_name
