...
def move_file_and_update_db(file_path, v_rec, videos):
    try:
        ...
        if new_path.exists():
            ...
            with db_lock:
                v_rec["path"] = str(new_path)
                save_json_file(JSON_DB, videos)
            logging.info("Crudo movido con exito tras confirmación HD: %s", file_path.name)
        else:
            logging.warning("Fuente no encontrada para mover: %s", file_path)
    except Exception as exc:
        logging.error("Error moviendo crudo %s tras verificacion: %s", file_path.name, exc)

def start_processing_verifier(video_id, client_secret_file, creds_cache_file, file_path, v_rec, videos):
    client_secret_file = Path(client_secret_file)
    creds_cache_file = Path(creds_cache_file)
    ...
    def bg_verify():
        ...
        if processing_ok is False:
            ...
            move_file_and_update_db(file_path, v_rec, videos)
...
def upload_video(youtube, file_path, upload_metadata, publish_at_dt, client_secret_file, creds_cache_file, v_rec, videos):
    ...
    verif_thread = start_processing_verifier(video_id, client_secret_file, creds_cache_file, file_path, v_rec, videos)

    return video_id, verif_thread
...
def main():
    parser = argparse.ArgumentParser(description="YouTube Uploader Granular")
    parser.add_argument("--video", help="Ruta absoluta del video especifico a subir.")
    parser.add_argument("--from-orchestrator", action="store_true", help="Saltar bloqueos si viene del orquestador.")
    ...
    # Si viene con video especifico, filtramos la base de datos
    if args.video:
        target_path = str(Path(args.video).resolve())
        videos = [v for v in videos if str(Path(v["path"]).resolve()) == target_path]
        if not videos:
            logging.error(f"El video especificado no esta en la base de datos: {args.video}")
            return
        logging.info(f"Modo Granular: Procesando UNICAMENTE {args.video}")
    ...
    while True:
        ...
        while True:
            ...
            # Mantenemos referencia del hilo para hacer join al final
            if isinstance(result, tuple) and len(result) == 2:
                video_id, verif_thread = result
                video["youtube_id"] = video_id
                if verif_thread:
                    active_threads.append(verif_thread)
            else:
    ...
    if active_threads:
        logging.info("Esperando que las %s tareas críticas en paralelo terminen de confirmar el procesamiento HD en Youtube...", len(active_threads))
        for t in active_threads:
            t.join()
        logging.info("Todas las confirmaciones paralelas en el uploader principal terminaron y se evacuo la carpeta.")
